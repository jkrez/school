...
class QLearningAgent(ReinforcementAgent):
  ...
  def getPolicy(self, state):
    ...
    maxActions = []
    maxActionTemp = -sys.maxint
    for action in self.getLegalActions(state):
      maxActionTemp = self.getQValue(state, action)
      if maxActionTemp > maxAction:
        maxActions[:] = []
        maxActions.append(action)
        maxAction = maxActionTemp

      elif maxActionTemp == maxAction:
        maxActions.append(action)
  ...
  def getAction(self, state):
    ...
    bestAction = None
    doExplore = util.flipCoin(self.epsilon)
    if doExplore:
