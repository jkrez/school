...
class InferenceModule:
  ...
  def getCapturedPosition(self, ghostIndex):
    return (2 * ghostIndex + 1, 1)
...
class ExactInference(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    if noisyDistance == 999:
        allPossible[self.getCapturedPosition()] = 1.0
    else:
...
class ParticleFilter(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    noisyDistance = observation
    allPossible = util.Counter()
    # If no observation then all ghosts in jail.
    if noisyDistance == 999:
        self.particles = [self.getCapturedPosition() for i in range(self.numParticles)]
    else:
        # construct beliefs based on emissionModel and current particle positions.
        for p in self.particles:
            trueDistance = util.manhattanDistance(p, pacmanPosition)
            allPossible[p] += emissionModel[trueDistance]
        # Make sure there are some beliefs... or uniform init.
        if allPossible.totalCount() != 0:
          # Take numParticles samples of the allPossible belief state.
          self.particles = []
  ...
  def elapseTime(self, gameState):
    ...
    for particle in self.particles:
      newParticles.append(util.sample(self.getPositionDistribution(self.setGhostPosition(gameState, particle))))
  ...
  def getBeliefDistribution(self):
    ...
    """
    "*** YOUR CODE HERE ***"
    # Sample each particle position and normalize to get belief of ghosts location.
...
class JointParticleFilter:
  ...
  def elapseTime(self, gameState):
    ...
    for oldParticle in self.particles:
      ...
      "*** YOUR CODE HERE ***"
      for i in range(self.numGhosts):
        newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, list(oldParticle)),
                                                   i+1, self.ghostAgents[i])
  ...
  def observeState(self, gameState):
    ...
    for particle in self.particles:
      for ghost in range(self.numGhosts):
        ...
        weights[ghost][particle[ghost]] += emissionModels[ghost][trueDistance]
    
    # Resample when each particle is weighted zero.
    totalWeight = 0
    ...
    for i in range(self.numParticles):
      ...
      for ghost in range(self.numGhosts):
        if noisyDistances[ghost] != 999:
          ...
          newParticle.append(tuple([2 * ghost + 1, 1]))
      newParticles.append(tuple(newParticle))
    self.particles = newParticles
